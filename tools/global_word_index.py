...
def query_word_list(word_list):
    ...
    index = query_word_index(word_list)  # _redis
    ...
    if len(uncached_words) > 0:
        ...
        if len(un_index_words) > 0:
            ...
            for loc, loc_word in enumerate(un_index_words):
                new_record.append(GlobalWordIndex(id=id_list[loc], Word=loc_word[1]))
...
def index_to_word(index):
    ...
    for loc, word in enumerate(word_list):
        if word:
            ...
            try:
                word = GlobalWordIndex.objects.get(id=index[loc])

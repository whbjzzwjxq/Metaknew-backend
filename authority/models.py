...
class AuthorityCount(models.Model):
    ...
    # 拥有修改状态权限的用户
    Manager = ArrayField(models.IntegerField(), db_column='Manager', default=list, db_index=True)
    # 拥有完整权限的用户
    Collaborator = ArrayField(models.IntegerField(), db_column='Coll', default=list, db_index=True)
    # 分享的用户
    SharedTo = ArrayField(models.IntegerField(), db_column='ShareTo', default=list, db_index=True)
    # 可以无偿使用的用户
    FreeTo = ArrayField(models.IntegerField(), db_column='FreeTo', default=list, db_index=True)
